...
class Sprite
{
    ...
    float scale;
    RectF base;
    RectF texture_region;
    PointF translation;

    public Sprite(float x, float y, float width, float height, RectF texture_region)
    {
        this.texture_region = texture_region;

        // Initialise our intital size around the 0,0 point
        base = new RectF(x ,y , x+width, y+height);
    ...
    public float[] getUVs(){
        return new float[]{
                texture_region.left, texture_region.top,
                texture_region.left, texture_region.bottom,
                texture_region.right, texture_region.bottom,
                texture_region.right, texture_region.top
        };
    }
